...
for i in $clusters ; do
	endpoint=$(aws rds --region sa-east-1 --profile destaxa describe-db-clusters --db-cluster-identifier $i |jq '.DBClusters[] .ReaderEndpoint' |tr -d \")
	username=$(aws rds --region sa-east-1 --profile destaxa describe-db-clusters --db-cluster-identifier $i |jq '.DBClusters[] .MasterUsername' |tr -d \")
	database=$(aws rds --region sa-east-1 --profile destaxa describe-db-clusters --db-cluster-identifier $i |jq '.DBClusters[] .DatabaseName' |tr -d \")
...
aws s3 --region sa-east-1 --profile destaxa cp dump_all.sh  s3://rds-backups-automation/pg_dump/dump_all.sh
