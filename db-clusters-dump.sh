...
for i in $clusters ; do
	...
	echo "PGPASSFILE=~/.pgpass pg_dump --create -s -h $i -U $username -d $database | aws s3 cp - s3://rds-backups-automation/pg_dump/backup_$i-$date.sql" |tee -a dump_all.sh 2>&1 1>/dev/null
	chmod +x dump_all.sh
done
